...
D3 is now socketed on the right below the ESP32. Its holes are 1.05 mm for the selected socket; do not use the C.3 drill files. Actual LED lead/contact retention must be qualified before ordering a kit-production batch. 3D models are nominal and do not certify physical fit.
...
Assembly uses the root project's C.4 BOM and guide. The separate power module is unchanged; J1 requires coordinated regulated 5 V, 3.3 V and 3.2 V rails. Never connect raw battery, the old power module, or USB and J1 simultaneously. Battery, enclosure, firmware and student-product release are not included in this fabrication package.

Earlier C.2/C.3 files are archived under `../archive/`; do not combine revisions.
