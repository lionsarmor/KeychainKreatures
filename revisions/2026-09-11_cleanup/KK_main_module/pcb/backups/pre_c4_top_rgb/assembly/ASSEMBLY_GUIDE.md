# Keychain Kreatures C.3 RGB — prototype assembly and bring-up

This guide is for an adult-supervised engineering prototype, not a released student product. Use the C.3 RGB BOM (98 electrical positions) and root-level one-sheet schematic. The board is 80 × 115 mm. The preassembled MCU, display and SD reader are the only surface-mount assemblies students handle; all main-board solder joints are through-hole. No battery, charger or regulator is fitted to this main PCB.

C.3 adds 25 **bare, unpopulated** plated debug holes; follow the [probe-face map and safety notes](C3_DEBUG_GUIDE.md). J4/J5 now use **B2B-PH-K-S(LF)(SN)** top-entry connectors, not the old S2B side-entry parts. PHR-2 mating plugs are unchanged. Allow about 8 mm mated connector height plus wire bends and insertion clearance behind the PCB. Capacitor stripes match the hatched negative half; the '+' mark identifies positive. IC/socket notches and pin-1 dots must match the board pin-1 markings. Transistor lead order is part-specific; do not substitute a different TO-92 part by body shape alone. D1's K mark identifies its IR cathode, not an RGB pin.

## Before assembly

1. Compare PCB revision, BOM and schematic: **C.3 RGB engineering prototype**. C.2's Q5/Q6 MOSFET and R36/R38 selections remain unchanged; J4/J5 are the top-entry parts. Do not mix older transistor or connector selections into this board.
2. Print the front and mirrored rear fit/assembly drawings at actual size. Confirm 80 x 115 mm outline with a ruler. A viewer at 67% is not a dimensional check. The user's earlier component fit check is accepted provisionally; dry-fit the new RGB section, moved bottom mounting holes, rear placement and assembled height before soldering.
...
| D3 — WP154A4SEJ3VBDZGW/CA | Front: 1 red cathode, 2 common anode (+5 V), 3 blue cathode, 4 green cathode. Form native 1.27 mm leads to the footprint's 2.54 mm pitch using a fixture; support leads near the body. Dry-fit a sample without forcing it. |
...
The 3D render has no qualified formed-lead D3 model and does not prove module/wire clearance. Print the fit sheets and check the actual LED, socket, display underside and JST plugs before soldering. Record all physical tests in [C3_BENCH_TEST_RECORD.csv](C3_BENCH_TEST_RECORD.csv).
