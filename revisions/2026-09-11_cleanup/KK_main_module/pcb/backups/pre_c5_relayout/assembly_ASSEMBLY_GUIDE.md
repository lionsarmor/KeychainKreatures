# Keychain Kreatures C.4 RGB — prototype assembly and bring-up

This guide is for an adult-supervised engineering prototype, not a released student product. Use the C.4 RGB BOM (98 electrical positions) and root-level one-sheet schematic. The board is 80 × 115 mm. D3 is socketed just below the ESP32 on the right; the other component positions are unchanged from C.3. The preassembled MCU, display and SD reader are the only surface-mount assemblies students handle; all main-board solder joints are through-hole. No battery, charger or regulator is fitted to this main PCB.

C.4 retains 25 **bare, unpopulated** plated debug holes; follow the [probe-face map and safety notes](C4_DEBUG_GUIDE.md). J4/J5 retain **B2B-PH-K-S(LF)(SN)** top-entry connectors, not the old S2B side-entry parts. PHR-2 mating plugs are unchanged. Allow about 8 mm mated connector height plus wire bends and insertion clearance behind the PCB. Capacitor stripes match the hatched negative half; the '+' mark identifies positive. IC/socket notches and pin-1 dots must match the board pin-1 markings. Transistor lead order is part-specific; do not substitute a different TO-92 part by body shape alone. D1's K mark identifies its IR cathode, not an RGB pin.

## Before assembly

1. Compare PCB revision, BOM and schematic: **C.4 RGB engineering prototype**. C.2's Q5/Q6 MOSFET and R36/R38 selections remain unchanged; J4/J5 are the top-entry parts. Do not mix older transistor or connector selections into this board.
2. Print the front and mirrored rear fit/assembly drawings at actual size. Confirm 80 x 115 mm outline with a ruler. A viewer at 67% is not a dimensional check. The user's earlier component fit check is accepted provisionally; dry-fit the relocated RGB/socket, modules, rear placement and assembled height before soldering.
...
1. Sort the 43 resistors by value with a meter, then populate the back using the by-reference BOM. Form axial leads to 10.16 mm without pulling on the body.
...
C.4 reroutes four LED connections and three nearby motor/backlight branches. Three GND pads (SW7:C, SW9:D, C4:2) use explicit 0.6 mm ground traces instead of incomplete thermal pour connections. No new DRC-category suppression was added. Bare debug holes remain unpopulated; motor/speaker stay removable at their JST plugs. Passives and small transistors remain soldered normally.
