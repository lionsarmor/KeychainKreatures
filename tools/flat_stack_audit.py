"""Fail-closed C6/P4 static audit; separate board outlines and mounting patterns."""
...
report={'scope':'Static flat-part/outline/width checks only; no powered or delivered-component qualification','boards':{},'physical_holds':['Independent power-board supports and JST wire-bend fit; no common mounting pattern','Seller screen, ESP32 and SD module dimensions','Horizontal capacitor lead forming and ripple performance','Battery location / case depth / RF test','Factory DFM / component supply approval before ordering']}
boards=[]
for target,stem in [(MAIN,'KK_main_module'),(POWER,'KK_power_module')]:
    ...
    dimensions=[96,105] if target==MAIN else [50,50]
    expected_holes={(4,4),(92,4),(4,101),(92,101)} if target==MAIN else {(3,3),(47,3),(3,47),(47,47)}
    assert all(abs(a-z)<.06 for a,z in zip(size,dimensions)),size
    assert holes.keys()=={'H1','H2','H3','H4'}
    assert {tuple(h['xy_mm']) for h in holes.values()}==expected_holes
...
rows=[];design=json.loads((POWER/'design.json').read_text());parts={p['ref']:p for p in design['components']}
...
assert len(rows)==108
with (POWER/'assembly/P4_BOM_AND_PLACEMENT_REVIEW.csv').open('w',newline='') as f:
...
(ROOT/'docs/CURRENT_STATIC_AUDIT.json').write_text(json.dumps(report,indent=2)+'\n')
